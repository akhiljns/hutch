// Command rabbitmq-example wires a hutch worker pool and publisher to RabbitMQ
// with isolated connections and graceful shutdown. It needs a running broker:
//
...
import (
	...
	"github.com/akhiljns/hutch/connectors/rabbitmq"
)
...
func main() {
	...
	// Separate connections for consuming and publishing == isolation.
	sub, err := rabbitmq.NewSubscriber(url, rabbitmq.WithLogger(log.Default()))
	if err != nil {
		log.Fatalf("subscriber: %v", err)
	}
	prod, err := rabbitmq.NewProducer(url, rabbitmq.WithLogger(log.Default()))
	if err != nil {
	...
	if err := pool.Handle("orders", hutch.Handle(func(_ context.Context, o Order) error {
		...
		return nil
	}), hutch.OnError(hutch.Reject)); err != nil {
